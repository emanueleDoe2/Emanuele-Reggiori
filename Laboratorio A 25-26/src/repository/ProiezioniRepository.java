...
/**
 * Gestisce il caricamento e il salvataggio delle proiezioni su file CSV.
 */
public class ProiezioniRepository {
	...
	/**
	 * Carica da file tutte le proiezioni valide.
	 *
	 * @return Elenco (ArrayList) dei risultati ottenuti.
	 */
			...
			while (scanner.hasNextLine()) {
				...
				if (!riga.isBlank()) {
					...
					try {
						...
						double costoBiglietto = Double.parseDouble(campi[7].trim());
	...
	/**
	 * Salva su file una nuova proiezione (in fondo).
	 *
	...
	public void salvaProiezione(Proiezione proiezione) {
		...
		try {
			...
			writer.write("\"" + proiezione.getDataOra().format(FORMATTER) + "\"," + "\""
					+ proiezione.getFilm().getTitolo() + "\"," + proiezione.getFilm().getGenere() + "," + "\""
					+ proiezione.getFilm().getRegista() + "\"," + proiezione.getFilm().getAnno() + ","
					+ proiezione.getFilm().getDurata() + "," + proiezione.getFilm().getEtaMinima() + ","
					+ proiezione.getCostoBiglietto() + System.lineSeparator());
	...
	/**
	 * Riscrive su file l’elenco completo delle proiezioni. Necessario dopo la
	 * cancellazione delle proiezioni.
	 *
	...
	public void salvaProiezioni(ArrayList<Proiezione> proiezioni) {
		...
		try {
			...
			writer.write("data_ora_proiezione,titolo_film,genere,regista,anno,durata_minuti,eta_minima,prezzo_biglietto"
